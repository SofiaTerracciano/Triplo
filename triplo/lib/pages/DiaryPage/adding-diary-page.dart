...
class AddingDiaryPage extends StatefulWidget {
  ...
  const AddingDiaryPage({super.key, required this.trekkingId});
...
class _AddingDiaryPageState extends State<AddingDiaryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<Users?>>(
      future: userController.getFollowing(userController.currentUser!.uid),
      builder: (context, snapshot) {
        ...
        final following = snapshot.data!.whereType<Users>().toList();
                                                ...
                                                Positioned.fill(
                                                  child: Center(
                                                    child: InkWell(
                                                      onTap: () {
                                                        setState(() {
                                                          final index = trekking
                                                              .challenges
                                                              .indexOf(c);
                                                          challenges.remove(c);
                                                          selectedChallengeIndexes
                                                              .remove(index);
                                                        });
                                                      },
                                                      child: Container(
                                ...
                                children: [
                                  SizedBox(
                                    ...
                                    child: FutureBuilder<List<String>>(
                                      future: Future.wait(
                                        trekking.challenges.map(
                                          (c) => trekkingController
                                              .getDownloadUrl(c),
                                        ),
                                      ),
                                      builder: (_, snapshot) {
