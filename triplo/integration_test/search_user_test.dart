...
void main() {
  ...
  testWidgets('User can search another user and open the public profile', (
      WidgetTester tester,
      ) async {
    await app.main();
    await tester.pumpAndSettle();
    ...
    await tester.tap(find.byKey(const Key('UserModeFilter')));
    await tester.pumpAndSettle();
    ...
    await waitFor(
      tester,
      find.byKey(const Key('userResult_0')),
      maxSeconds: 20,
    );

    expect(find.byKey(const Key('userResult_0')), findsOneWidget);

    await tester.tap(find.byKey(const Key('userResult_0')));
    await tester.pumpAndSettle();

    expect(find.byKey(const Key('userPublicPage')), findsOneWidget);
  });
}